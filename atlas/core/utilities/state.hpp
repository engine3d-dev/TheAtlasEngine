...
namespace atlas {
    ...
    namespace detail {
        ...
        // anything in the detail namespace should not be invoked by the application code
        ...
        template<typename T>
        struct has_member_fn<T, std::void_t<decltype(T::foo)>> : std::true_type {};
    };
    ...
    void register_start(UObject* p_instance, const UCallback& p_callable) {
        static_assert(std::is_member_pointer_v<UCallback>,
                          "Cannot register a function that is not a member function of a class object");
        detail::poll_start([p_instance, p_callable]() { (p_instance->*p_callable)(); });
    }
    ...
    void register_ui(UObject* p_instance, const UCallback& p_callable) {
        static_assert(std::is_member_pointer_v<UCallback>,
                          "Cannot register a function that is not a member function of a class object");
        detail::poll_ui_update([p_instance, p_callable]() { (p_instance->*p_callable)(); });
    }
