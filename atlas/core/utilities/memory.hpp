...
namespace atlas::memory {
    ...
    namespace detail {
         ...
         *
         * This structure manages the lifetime of reference-counted objects by
         * tracking strong and weak references. It also stores the memory
         * allocator and destroy function used to clean up the object when no
        ...
        struct ref_info {
             ...
             *
             * Always returns the total size of the object wrapped in a ref
             * count object. Thus the size should normally be greater than
             * sizeof(T). Expect sizeof(T) + sizeof(ref_info) and anything else
             * the ref count object may contain.
             *
             * If a nullptr is passed to the destroy function, it returns the
             * object size but does not destroy the object.
             */
        ...
        /**
         * @brief A wrapper that contains both the ref_info and the actual
         * object
         *
         * This structure keeps the control block and managed object together in
         * memory.
         *
     ...
     *
     * strong_ptr is a smart pointer that maintains shared ownership of an
     * object through a reference count. It is similar to std::shared_ptr but
     ...
     * the pointer will never be null. For nullable references, use
     * optional_ptr.
     *
     * Example usage:
     *
    ...
    public:
        ...
        /**
         * @brief Move constructor that intentionally behaves like a copy
         * constructor for safety
         *
         * This move constructor deliberately performs a full copy operation
         * rather than transferring ownership. This is a safety feature to
         * prevent potential undefined behavior that could occur if code
         * accidentally accessed a moved-from strong_ptr.
         *
         * After this operation, both the source and destination objects remain
         * in valid states, and the reference count is incremented by 1. This
         * ensures that even if code incorrectly continues to use the source
         * object after a move, no undefined behavior will occur.
         *
         * @param p_other The strong_ptr to "move" from (actually copied for
         * safety)
         */
        ...
        /**
         * @brief Move assignment operator that behaves like a copy assignment
         * for safety
         *
         * This move assignment operator deliberately performs a full copy
         * operation rather than transferring ownership. This is a safety
         * feature to prevent potential undefined behavior that could occur if
         * code accidentally accessed a moved-from strong_ptr.
         *
         * After this operation, both the source and destination objects remain
         * in valid states, and the reference count is incremented by 1. This
         * ensures that even if code incorrectly continues to use the source
         ...
         *
         * `std::shared_ptr` provides an alias constructor that accepts any
         * `void*` which is UB if that `void*` doesn't have the same lifetime as
         * the object referenced by the `std::shared_ptr`. Users attempting to
         * do this will get a list of constructors that failed to fit. This is
         * not a good error message for users. Instead, we provide a
         * static_assert message in plain english that explains why this
         * overload fails at compile time.
         *
         ...
         *
         * This constructor creates a strong_ptr that points to a member of an
         * object managed by another strong_ptr. The resulting strong_ptr shares
         * ownership with the original strong_ptr, keeping the entire parent
         * object alive.
         *
         * This version is only enabled for non-array members to prevent
         * potential undefined behavior when accessing array elements directly.
         * Use the array-specific versions instead.
         *
         * Example usage:
         * ```
         * struct container {
         ...
         * @param p_member_ptr Pointer-to-member identifying which member to
         * reference
         */
         ...
         *
         * This constructor creates a strong_ptr that points to an element of an
         * array member in an object managed by another strong_ptr. It performs
         * bounds checking to ensure the index is valid.
         *
         * Example usage:
         * ```
     ...
     * safely obtain a strong_ptr to itself. The object must inherit from this
     * class and be managed by a strong_ptr created via make_strong_ptr.
     *
     * Example usage:
     * ```cpp
    ...
    public:
         ...
         * @throws hal::bad_weak_ptr if this object is not managed by a
         * strong_ptr
         */
    ...
    protected:
         ...
         *
         * Note: The weak_ptr is not copied - each object gets its own weak
         * reference
         */
     ...
     *
     * weak_ptr provides a non-owning reference to an object managed by
     * strong_ptr. It can be used to break reference cycles or to create an
     * optional_ptr.
     *
     * A weak_ptr doesn't increase the strong reference count, so it doesn't
     * prevent the object from being destroyed when the last strong_ptr goes
     * away.
     *
     * Example usage:
     * ```
    ...
    public:
         ...
         * @brief Converting copy constructor
         *
         * Creates a weak_ptr of T from a weak_ptr of U where U is convertible
         * to T.
         *
         * @tparam U A type convertible to T
         * @param p_other The weak_ptr to copy from
    ...
    /**
     * @brief Optional, nullable, smart pointer that works with
     * `hal::strong_ptr`.
     *
     ...
     *
     * Use optional_ptr when you need a nullable reference to a
     * reference-counted object, such as:
    ...
    public:
        ...
        /**
         * @brief Access the contained value, throw if not engaged (const
         * version)
         *
         * @return A copy of the contained strong_ptr
         * @throws hal::bad_optional_ptr_access if *this is disengaged
         ...
         * @return A copy of the contained strong_ptr, converted to the target
         * type
         * @throws hal::bad_optional_ptr_access if *this is disengaged
         */
        template<typename U>
     ...
     * containing a strong_ptr to it. Otherwise, it returns an empty
     * optional_ptr.
     *
     * @tparam T The type of the referenced object
     * @return An optional_ptr that is either empty or contains a strong_ptr
    ...
    /**
     * @brief Factory function to create a strong_ptr with automatic
     * construction detection
     *
     * This is the primary way to create a new strong_ptr. It automatically
     * detects whether the target type requires token-based construction (for
     * classes that should only be managed by strong_ptr) or supports normal
     * construction.
     *
     ...
     * 4. Initializes enable_strong_from_this support if the type inherits from
     * it
     * 5. Returns a strong_ptr managing the newly created object
     *
     * **Token-based construction**: If a class constructor takes
     * `strong_ptr_only_token` as its first parameter, this function
     * automatically provides that token, ensuring the class can only be
     * constructed via make_strong_ptr.
     *
     * **Normal construction**: For regular classes, construction proceeds
     * normally with the provided arguments.
     *
     * **enable_strong_from_this support**: If the constructed type inherits
     * from `enable_strong_from_this<T>`, the weak reference is automatically
     * initialized to enable `strong_from_this()` and `weak_from_this()`
     * functionality.
     *
     * Example usage:
     *
