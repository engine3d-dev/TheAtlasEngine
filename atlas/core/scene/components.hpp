...
namespace atlas{
    /**
     * @note scene_object
     * @note scene_object is specifically for referencing components to the scene node they may be part of
    ...
    */
    struct MeshComponent{
        ...
        MeshComponent(const std::string& p_filepath) : Filepath(p_filepath), MeshMetadata(p_filepath) {}
    ...
    struct RigidBody3D{
        ...
        void* BodyAtRuntime = nullptr; // for storing the actual data of the body
    };

    //! @note Our interpretation of the BoxCollider that will get interpreted based on physics API's enabled
    ...
    public:
        ...
        PerspectiveCamera(glm::vec3 position = glm::vec3(0.0f, 1.50f, 0.0f), glm::vec3 up = glm::vec3(0.0f, -1.0f, 0.0f), float yaw = YAW, float pitch = PITCH) : Front(glm::vec3(0.0f, 0.0f, 1.0f)), MovementSpeed(5.f), MouseSensitivity(0.1f), Zoom(ZOOM), camera_mouse_sensitivity(0.1f){
            Position = position;
            WorldUp = up;
            Yaw = yaw;
            Pitch = pitch;
            AspectRatio = application_instance::get_aspect_ratio();
            updateCameraVectors();
        }
    ...
    public:
        void UpdateProjView(){
            //! TODO: Eventually we will have camera configurations that will utilize this.
            Projection = glm::perspective(glm::radians(Zoom), AspectRatio, .1f, 50000.f);
            View = GetViewMatrix();
        }
