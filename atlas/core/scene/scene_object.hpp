...
namespace atlas{
    ...
    */
    ...
    public:
        ...
        scene_object(flecs::world* p_registry, const std::string& p_tag) : m_entity(p_registry, p_tag){
            //! @note Because flecs the ecs framework uses components to indicate which entities you want to query from.
        ...
        scene_object(const flecs::entity& p_Entity) : m_entity(p_Entity) {}

        ~scene_object(){
            console_log_fatal("Scene Object Tag = {} HAS DESTRUCTED!!!", m_entity.get<Tag>()->TagMetadata);
        ...
        std::string get_tag() const {
            return m_entity.get<Tag>()->TagMetadata;
        }
