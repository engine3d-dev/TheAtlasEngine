...
namespace atlas{
     ...
     * @note Because Render and Physics thread may differ here is how I see them working together for the time being
     * 
     * Thread Abstraction (In general should have)
     *  - Keeps track of Thread Policy (setting scheduling policy for our specific threads)
     *  - Can also set prioritized policies for which thread to be scheduled 
     * 
     * Render Thread
     * - Handle next frame of the rendering that happens
     * - Handle in setting specific requests from the user and differentiate these requests to either render or physics thread.
    */
   class thread{
    public:
        thread() = delete;
