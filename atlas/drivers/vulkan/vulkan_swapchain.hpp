...
namespace atlas::vk{
    ...
    */
    ...
    public:
        ...
        vk_swapchain(vk_physical_driver p_physical, vk_driver p_driver, VkSurfaceKHR p_surface);
    ...
    private:
        ...
        void submit_and_write_command_buffer(VkCommandBuffer* p_command_buffers) override;
    ...
    private:
        ...
        VkPresentModeKHR select_compatible_present_mode(const VkPresentModeKHR& p_request_mode, const std::vector<VkPresentModeKHR>& p_modes);
        VkExtent2D select_valid_extent(const VkSurfaceCapabilitiesKHR& p_surface_capabilities);

        //! @note Search Memory Type
        uint32_t select_memory_type(VkPhysicalDeviceMemoryProperties p_memory_properties, uint32_t p_type_filter, VkMemoryPropertyFlags p_property_flag);
        ...
        */
        ...
        VkFormat select_supported_format(VkPhysicalDevice p_physical, const std::vector<VkFormat>& p_formats, VkImageTiling p_tiling, VkFormatFeatureFlags p_feature_flags);
    ...
    private:
        ...
        std::vector<VkCommandBuffer> m_swapchain_command_buffers; // command buffers per swapchain
        ...
        std::vector<VkSemaphore> m_semaphores_images_available; // semaphores for when images are available
        std::vector<VkSemaphore> m_semaphores_render_completed; // semaphores when working on the current frame has been completed
        
        std::vector<VkFence> m_swapchain_fences; // fences for when frames in flight
        std::vector<VkFence> m_swapchain_in_flight_fences; // images fences for when we currently have images currently in flight
        ...
        uint32_t m_current_image_index = 0; // contains the index of the image we are currently working through
    };
};