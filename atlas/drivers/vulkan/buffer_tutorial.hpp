...
namespace atlas::vk{
    ...
    */
    ...
    public:
        ...
        */
        BufferTutorial(uint64_t p_size, uint32_t p_count, VkBufferUsageFlags p_usage, VkMemoryPropertyFlags p_memory_property_flags, uint64_t p_min_offset_alignment=1);

        void map(VkDeviceSize p_Size = VK_WHOLE_SIZE, uint64_t p_Offset = 0);
        void unmap();

        void write_to_buffer(void* data, uint64_t p_Size = VK_WHOLE_SIZE, uint64_t p_Offset = 0);
        void flush(uint64_t p_Size = VK_WHOLE_SIZE, uint64_t offset = 0);
        VkDescriptorBufferInfo descriptor_info(uint64_t p_Size = VK_WHOLE_SIZE, uint64_t p_Offset = 0);
