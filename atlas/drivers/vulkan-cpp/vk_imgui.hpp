...
namespace atlas::vk {
    ...
    public:
        ...
        // If users have custom components they want to register to the UI, then do:
        /*  
            template<typename UComponent, typename UType>
            void custom_hud_component(UComponent* p_component,, UType p_type) {
                // then prob do something lik this: use template to associate struct component and the data, with type specified
            ...
            const custom_component_health* player_hp = some_entity.get<custom_component_heatlh>();
            ...
            2.) Ease of usability with minimal specifications on user-side for getting HUD's to work
            3.) Making a class be optional, meaning have this be through imgui's API's as a function
            4.) Asking if its needed to be represented as a class
            atlas::ui::hud::health_bar(player_hp, &custom_component_health::health_data)
        */
