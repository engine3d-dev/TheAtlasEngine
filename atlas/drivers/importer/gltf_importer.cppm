...
namespace atlas {
    ...
    public:
        gltf_importer(const std::string& p_path, bool p_flip) : m_flip(p_flip) {
            m_load_successful = load(p_path);
        }

        bool load(const std::string& p_path) {
            if (!std::filesystem::exists(p_path)) {
                console_log_error("GLTF Error: Path does not exist: {}", p_path);
                return false;
            }

            size_t base_color_uv_index = 0;

            // 1. Create the Parser configuration (enable SIMD parsing if available)
            ...
            if (data.error() != fastgltf::Error::None) {
                ...
                console_log_info("GLTF Error Message: {}", fastgltf::getErrorMessage(data.error()));
                m_load_successful = false;
                return m_load_successful;
            }

            auto asset = parser.loadGltf(data.get(), filepath.parent_path(), gltf_options);
            if (asset.error() != fastgltf::Error::None) {
                console_log_error("GLTF Error: Parsing failed with code {}", static_cast<size_t>(asset.error()));
                console_log_info("GLTF Error Message: {}", fastgltf::getErrorMessage(asset.error()));
                m_load_successful = false;
                return m_load_successful;
            }
            ...
            for(const auto& mesh : model.meshes) {
                for(const auto& primitive : mesh.primitives) {
                    ...
                    // Get output primitive mesh texture color index (base_color_uv_index)
                    auto* primitive_mesh = primitive.findAttribute("POSITION");

                    if(primitive.materialIndex.has_value()) {
                        auto& material = model.materials[primitive.materialIndex.value()];

                        auto& base_color_texture = material.pbrData.baseColorTexture;

                        if(base_color_texture.has_value()) {
                            auto& texture = model.textures[base_color_texture->textureIndex];

                            if(!texture.imageIndex.has_value()) {
                                return false;
                            }

                            if (base_color_texture->transform and base_color_texture->transform->texCoordIndex.has_value()) {
                                base_color_uv_index = base_color_texture->transform->texCoordIndex.value();
                            }
                            else {
                    ...
                    // Required to have a primitive that contains the meshes position
                    ...
                    auto& position_accessor = model.accessors[primitive_mesh->accessorIndex];
                    ...
                    const auto& pos_accessor = model.accessors[primitive_mesh->accessorIndex];
                    ...
                    auto uv_attribute = primitive.findAttribute(std::format("TEXCOORD_{}", base_color_uv_index));
                    bool has_uv = (uv_attribute != primitive.attributes.end());

                    auto normal_attribute = primitive.findAttribute("NORMAL");
                    bool has_normals = (normal_attribute != primitive.attributes.end());

                    fastgltf::iterateAccessorWithIndex<fastgltf::math::fvec3>(model, position_accessor, [&](fastgltf::math::fvec3 pos, size_t index){
                        vk::vertex_input vertex{};
                        vertex.position = {pos.x(), pos.y(), pos.z()};

                        if(has_normals) {
                            const auto& normal_accessor = model.accessors[normal_attribute->accessorIndex];
    
                            fastgltf::math::fvec3 norm = fastgltf::getAccessorElement<fastgltf::math::fvec3>(model, normal_accessor, index);
                    ...
                    if(primitive.indicesAccessor.has_value()) {
                        const auto& index_accessor = model.accessors[primitive.indicesAccessor.value()];
                        m_indices.reserve(m_indices.size() + index_accessor.count);

                        fastgltf::iterateAccessor<uint32_t>(model, index_accessor, [&](uint32_t index) {
