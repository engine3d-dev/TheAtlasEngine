...
inline void ConsoleLogTrace(spdlog::format_string_t<T...> fmt, T&&... args){
    engine3d::ConsoleEngineLogger::GetLogger("core")->trace(fmt, std::forward<T>(args)...);
}

template<typename... T>
inline void ConsoleLogWarn(spdlog::format_string_t<T...> fmt, T&&... args){
    engine3d::ConsoleEngineLogger::GetLogger("core")->warn(fmt, std::forward<T>(args)...);
}

template<typename... T>
inline void ConsoleLogInfo(spdlog::format_string_t<T...> fmt, T&&... args){
    engine3d::ConsoleEngineLogger::GetLogger("core")->info(fmt, std::forward<T>(args)...);
}

template<typename... T>
inline void ConsoleLogError(spdlog::format_string_t<T...> fmt, T&&... args){
    engine3d::ConsoleEngineLogger::GetLogger("core")->error(fmt, std::forward<T>(args)...);
}

template<typename... T>
inline void ConsoleLogFatal(spdlog::format_string_t<T...> fmt, T&&... args){
    engine3d::ConsoleEngineLogger::GetLogger("core")->critical(fmt, std::forward<T>(args)...);
}
