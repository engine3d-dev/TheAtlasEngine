...
namespace engine3d{
    namespace vk{
        ...
        static VKAPI_ATTR VkBool32 VKAPI_CALL DebugCallback(
            ...
            CoreLogWarn("Debug callback: {}\n", pCallbackData->pMessage);
            CoreLogWarn("\tSeverity {}", GetDebugSeverity(Severity));
            CoreLogWarn("\tType {}", GetDebugType(Type));
            CoreLogWarn("\tObjects");

            for(uint32_t i = 0; i < pCallbackData->objectCount; i++){
                CoreLogWarn("{} ", pCallbackData->pObjects[i].objectHandle);
            }
        ...
        void VulkanPipeline::InitializePipeline(){
            ...
            if(res != VK_SUCCESS){
                CoreLogError("vkCreateInstance() was not successful in InitializePipeline() in file Vulkan.cpp");
                assert(false);
            }
        ...
        void VulkanPipeline::CleanupPipeline(){
            ...
            if(!vkDestroyDebugUtilsMessenger){
                CoreLogError("Cannot fnid address of vkDestroyDebugUtilsMessenger");
            }
            ...
            if(!vkDestroySurface){
                CoreLogError("Cannot find address of vkDestroySurfaceKHR");
            }
