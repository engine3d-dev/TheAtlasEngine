...
namespace engine3d::vk{
    ...
    */
    void VulkanPhysicalDevice::InitializePhysicalDevice(){
        ...
        VkResult res = vkEnumeratePhysicalDevices(VulkanPipeline::GetVkInstance(), &devicesCount, nullptr);

        if(res != VK_SUCCESS){
            ConsoleLogError("vkEnumeratePhysicalDevices Error (1) = {}", VkResultToString(res));
        }
        ...
        res = vkEnumeratePhysicalDevices(VulkanPipeline::GetVkInstance(), &devicesCount, devices.data());
        if(res != VK_SUCCESS){
            ConsoleLogError("vkEnumeratePhysicalDevices Error (2) = {}", VkResultToString(res));
        }
        ...
        */
        for(uint32_t i = 0; i < devicesCount; i++){
            ...
            ConsoleLogInfo("Device Name {}", m_PhysicalDevices[i].deviceProperties.deviceName);
            ConsoleLogInfo("API Version: {}.{}.{}.{}", VK_API_VERSION_VARIANT(apiVer), VK_API_VERSION_MAJOR(apiVer), VK_API_VERSION_MINOR(apiVer), VK_API_VERSION_PATCH(apiVer));
            ConsoleLogInfo("Count of Family Queues: {}", numQFamilies);
        #endif
            ...
            for(uint32_t q = 0; q < numQFamilies; q++){
                ...
                res = vkGetPhysicalDeviceSurfaceSupportKHR(dev, q, VulkanPipeline::GetVkSurface(), &(m_PhysicalDevices[i].queueSupportPresent[q]));
            #ifdef ENGINE_DEBUG_BUILD
                ConsoleLogInfo("\t\tFamiy {} Num Queues: {} ", q, qFamProperties.queueCount);
                ConsoleLogInfo("\t\tGFX {}, Compute {}, Transfer {}, Sparse Binding {}", (flags & VK_QUEUE_GRAPHICS_BIT) ? "Yes" : "No", (flags & VK_QUEUE_COMPUTE_BIT) ? "Yes" : "No", (flags & VK_QUEUE_TRANSFER_BIT) ? "Yes" : "No", (flags & VK_QUEUE_SPARSE_BINDING_BIT) ? "Yes" : "No");
                if(res != VK_SUCCESS){
                    ConsoleLogError("vkGetPhysicalDeviceSurfaceSupporKHR Error = {}", VkResultToString(res));
                }
            #endif
            }

            uint32_t formatsCount = 0; // NumFormats
            res = vkGetPhysicalDeviceSurfaceFormatsKHR(dev, VulkanPipeline::VulkanPipeline::GetVkSurface(), &formatsCount, nullptr);
            if(res != VK_SUCCESS){
                ConsoleLogError("vkGetPhysicalDeviceSurfaceFormatsKHR (1) Error = {}", VkResultToString(res));
            }

            m_PhysicalDevices[i].surfaceFormats.resize(formatsCount);

            res = vkGetPhysicalDeviceSurfaceFormatsKHR(dev, VulkanPipeline::VulkanPipeline::GetVkSurface(), &formatsCount, m_PhysicalDevices[i].surfaceFormats.data());
            if(res != VK_SUCCESS){
                ConsoleLogError("vkGetPhysicalDeviceSurfaceFormatsKHR (2) Error = {}", VkResultToString(res));
            }

        #ifdef ENGINE_DEBUG_BUILD
            for(uint32_t j = 0; j < formatsCount; j++){
                const VkSurfaceFormatKHR& surfaceFormat = m_PhysicalDevices[i].surfaceFormats[j];
                ConsoleLogInfo("\t\tFormat {} color space {}", VkFormatToString(surfaceFormat.format), VkColorspaceToString(surfaceFormat.colorSpace));
            }
        #endif

            res = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(dev, VulkanPipeline::VulkanPipeline::GetVkSurface(), &(m_PhysicalDevices[i].surfaceCapabilities));
            if(res != VK_SUCCESS){
                ConsoleLogInfo("vkGetPhysicalDeviceSurfaceCapabilitiesKHR Error = {}", VkResultToString(res));
            }
            // else{
            //     ConsoleLogInfo("Image Usage Flag {}", VkImageUsageFlagsToString(m_PhysicalDevices[i].surfaceCapabilities.supportedUsageFlags));
            // }

            uint32_t presentModesCount = 0;
            res = vkGetPhysicalDeviceSurfacePresentModesKHR(dev, VulkanPipeline::VulkanPipeline::GetVkSurface(), &presentModesCount, nullptr);

            if(res != VK_SUCCESS){
                ConsoleLogInfo("vkGetPhysicalDeviceSurfacePresentModesKHR (1) Error = {}", VkResultToString(res));
            }

            res = vkGetPhysicalDeviceSurfacePresentModesKHR(dev, VulkanPipeline::VulkanPipeline::GetVkSurface(), &presentModesCount, m_PhysicalDevices[i].presentModes.data());
            if(res != VK_SUCCESS){
                ConsoleLogInfo("vkGetPhysicalDeviceSurfacePresentModesKHR (2) Error = {}", VkResultToString(res));
            }

        #ifdef ENGINE_DEBUG_BUILD
            ConsoleLogInfo("Number of Presentation Modes {}", presentModesCount);

            vkGetPhysicalDeviceMemoryProperties(dev, &(m_PhysicalDevices[i].memoryProperties));
            ConsoleLogInfo("Number memory types {}", m_PhysicalDevices[i].memoryProperties.memoryTypeCount);

            for(uint32_t j = 0; j < m_PhysicalDevices[i].memoryProperties.memoryTypeCount; j++){
                ConsoleLogInfo("{}: flags, {}: Heap Index{} ", j, VkMemoryPropertyFlagToString(m_PhysicalDevices[i].memoryProperties.memoryTypes[i].propertyFlags), m_PhysicalDevices[i].memoryProperties.memoryTypes[i].heapIndex);
            }

            ConsoleLogInfo("Num heap types {}", m_PhysicalDevices[i].memoryProperties.memoryHeapCount);
        #endif
    ...
    uint32_t VulkanPhysicalDevice::SelectDevice(VkQueueFlags ReqQueueFlag_t, bool IsSupportPresent){
        ...
        for(uint32_t i = 0; i < m_PhysicalDevices.size(); i++){
            for(uint32_t j = 0; j < m_PhysicalDevices[i].queueFamProperties.size(); j++){
                ...
                if((qFamProperties.queueFlags & ReqQueueFlag_t) and ((bool)m_PhysicalDevices[i].queueSupportPresent[j] == IsSupportPresent)){
                    m_DeviceIdx = i;
                    int queueFam = j;
                    // ConsoleLogInfo("Using GFX Device {} and Queue Family {}", m_DeviceIdx, queueFam);
        ...
        // ConsoleLogError("Required queue type {} and supports present {} was not found!", ReqQueueFlag_t, IsSupportPresent);
        return 0;
    }

    VkPhysicalDevice VulkanPhysicalDevice::Selected(){
        if(m_DeviceIdx < 0){
            ConsoleLogError("Physical device not selected!");
        }
    ...
    void VulkanLogicalDevice::InitializeLogicalDevice(){
        ...
        if(!g_PhysicalDevice.IsGeometryShaderSupported()){
            ConsoleLogError("Geometry Shader not supported!");
        }

        if(!g_PhysicalDevice.IsTesselationSupported()){
            ConsoleLogError("Tesselelation Shader not supported!");
        }
