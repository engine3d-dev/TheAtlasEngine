...
namespace engine3d::vk{
    void vk_check(VkResult result, const char* p_Tag, const char* p_Filename, uint32_t p_LineName, const char* p_FunctionName){
        if(result != VK_SUCCESS){
            ConsoleLogErrorWithTag("vulkan", "VkResult failed taking in {0} file: {1} --- Line: {2} --- In Function: {3}",p_Tag, p_Filename, p_LineName, p_FunctionName);
            ConsoleLogErrorWithTag("vulkan", "VkResult returned: {}", (int)result);
        }
    }
    ...
    void vk_check_format(VkFormat p_Format, const char* p_Filename, uint32_t p_LineName, const char* p_FunctionName){
        if(p_Format == VK_FORMAT_UNDEFINED){
            ConsoleLogErrorWithTag("vulkan", "VkFormat failed in file: {0} --- Line: {1} --- In Function: {2}", p_Filename, p_LineName, p_FunctionName);
            ConsoleLogErrorWithTag("vulkan", "VkFormat was assigned to VK_FORMAT_UNDEFINED");
        }
    }
