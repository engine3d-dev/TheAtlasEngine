...
namespace engine3d::vk{
    ...
    VulkanSwapchain::VulkanSwapchain(VulkanPhysicalDriver p_PhysicalDriver, VulkanDriver p_Driver, VkSurfaceKHR p_Surface) : m_PhysicalDriver(p_PhysicalDriver), m_Driver(p_Driver), m_CurrentSurface(p_Surface){
        ...
        OnCreate(ApplicationInstance::GetWindow().GetWidth(), ApplicationInstance::GetWindow().GetHeight());
    }

    void VulkanSwapchain::OnCreate(uint32_t p_Width, uint32_t p_Height){
        m_Width = p_Width;
        m_Height = p_Height;
        ...
        vk_check(vkGetPhysicalDeviceSurfaceFormatsKHR(m_PhysicalDriver, m_CurrentSurface, &format_count, nullptr), "vkGetPhysicalDeviceSurfaceFormatsKHR", __FILE__, __LINE__, __FUNCTION__);

        formats.resize(format_count);
        vk_check(vkGetPhysicalDeviceSurfaceFormatsKHR(m_PhysicalDriver, m_CurrentSurface, &format_count, formats.data()), "vkGetPhysicalDeviceSurfaceFormatsKHR", __FILE__, __LINE__, __FUNCTION__);
        ...
        vk_check(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(m_PhysicalDriver, m_CurrentSurface, &surface_capabilities), "vkGetPhysicalDeviceSurfaceCapabilitiesKHR", __FILE__, __LINE__, __FUNCTION__);

        //! @note Extracting number of presentation modes.
        uint32_t preset_modes_count;
        std::vector<VkPresentModeKHR> presentation_modes;
        vk_check(vkGetPhysicalDeviceSurfacePresentModesKHR(m_PhysicalDriver, m_CurrentSurface, &preset_modes_count, nullptr), "vkGetPhysicalDeviceSurfacePresentModesKHR", __FILE__, __LINE__, __FUNCTION__);
        presentation_modes.resize(preset_modes_count);
        vk_check(vkGetPhysicalDeviceSurfacePresentModesKHR(m_PhysicalDriver, m_CurrentSurface, &preset_modes_count, presentation_modes.data()), "vkGetPhysicalDeviceSurfacePresentModesKHR", __FILE__, __LINE__, __FUNCTION__);
        ...
        ConsoleLogWarnWithTag("vulkan", "Vulkan Swapchain Initialized Complete!!!");

        // ConsoleLogInfo("Vulkan2Showcase: Begin VulkanSwapchain Images Initialization");
        ConsoleLogInfoWithTag("vulkan", "Begin VulkanSwapchain Images Initialization");
        ...
        for(uint32_t i = 0; i < swapchain_images_count; i++){
            ...
            vk_check(vkCreateImageView(m_Driver, &image_view_create_info, nullptr, &m_ImagesForSwapchain[i].ImageView), "vkCreateImageView", __FILE__, __LINE__, __FUNCTION__);
        }

        // ConsoleLogWarn("Vulkan2Showcase: Image Initiated Completed!!!");
        ConsoleLogWarnWithTag("vulkan", "Image Initiated Completed!!!");
        ...
        VkCommandPoolCreateInfo cmd_pool_create_info = {
            ...
            .queueFamilyIndex = m_PhysicalDriver.GetQueueIndices().Graphics,
        };

        vk_check(vkCreateCommandPool(m_Driver, &cmd_pool_create_info, nullptr, &m_CommandPoolForSwapchain), "vkCreateCommandPool", __FILE__, __LINE__, __FUNCTION__);
        ...
        vk_check(vkAllocateCommandBuffers(m_Driver, &cmd_buffer_alloc_info, m_SwapchainCommandBuffers.data()), "vkAllocateCommandBuffers", __FILE__, __LINE__, __FUNCTION__);

        // ConsoleLogWarn("Vulkan2Showcase: Vulkan Swapchain CommandBuffers Initiated Completed!!");
        ConsoleLogWarnWithTag("vulkan", "Vulkan Swapchain CommandBuffers Initiated Completed!!");
        ...
        ConsoleLogInfoWithTag("vulkan", "Creating VulkanSwapchain RenderPass!");
        VkAttachmentDescription depth_attachments_description = {
            .format = SelectDepthFormat(m_PhysicalDriver),
        ...
        ConsoleLogWarnWithTag("vulkan", "Vulkan Swapchain Renderpass Initialized Completed!!!");
        ...
        ConsoleLogInfoWithTag("vulkan", "Begin Vulkan Swapchain Depth Images for swapchains Initializing");
        VkFormat selected_depth_format = SelectDepthFormat(m_PhysicalDriver);
        ...
        vkGetPhysicalDeviceMemoryProperties(m_PhysicalDriver, &memory_properties);
        ...
        for(size_t i = 0; i < m_DepthImagesForSwapchain.size(); i++){
            ...
            vk_check(vkCreateImage(m_Driver, &image_create_info, nullptr, &m_DepthImagesForSwapchain[i].Image), "vkCreateImage", __FILE__, __LINE__, __FUNCTION__);

            VkMemoryRequirements memory_requirements;
            vkGetImageMemoryRequirements(m_Driver, m_DepthImagesForSwapchain[i].Image, &memory_requirements);
            ...
            vk_check(vkAllocateMemory(m_Driver, &mem_alloc_info, nullptr, &m_DepthImagesForSwapchain[i].DeviceMemory), "vkAllocateMemory", __FILE__, __LINE__, __FUNCTION__);

            vk_check(vkBindImageMemory(m_Driver, m_DepthImagesForSwapchain[i].Image, m_DepthImagesForSwapchain[i].DeviceMemory, 0), "vkBindImageMemory", __FILE__, __LINE__, __FUNCTION__);
            ...
            vk_check(vkCreateImageView(m_Driver, &image_view_create_info, nullptr, &m_DepthImagesForSwapchain[i].ImageView), "vkCreateImageView", __FILE__, __LINE__, __FUNCTION__);
        }

        // ConsoleLogWarn("Vulkan2Showcase: Depth Images Initialized Completed!!");
        ConsoleLogWarnWithTag("vulkan", "Depth Images Initialized Completed!!");


        //! @note Setting up framebuffers for swapchain.
        // ConsoleLogInfo("Vulkan2Showcase: Begin Framebuffers Initialization!");
        ConsoleLogInfoWithTag("vulkan", "Begin Framebuffers Initialization!");
        ...
        for(size_t i = 0; i < GetImagesSize(); i++){
            ...
            vk_check(vkCreateFramebuffer(m_Driver, &fb_create_info, nullptr, &m_FramebuffersForSwapchain[i]), "vkCreateFramebuffer", __FILE__, __LINE__, __FUNCTION__);
        }

        // ConsoleLogWarn("Vulkan2Showcase: Vulkan Framebuffers Initialization Completed!!!!");
        ConsoleLogWarnWithTag("vulkan", "Vulkan Framebuffers Initialization Completed!!!!");
        ...
        ConsoleLogInfoWithTag("vulkan", "Begin Vulkan Sync Objects Initialization");
        ...
        for(size_t i = 0; i < MaxFramesInFlight; i++){
            vk_check(vkCreateSemaphore(m_Driver, &semaphore_create_info, nullptr, &m_SemaphoresForAvailableImages[i]), "vkCreateSemaphore (1)", __FILE__, __LINE__, __FUNCTION__);

            vk_check(vkCreateSemaphore(m_Driver, &semaphore_create_info, nullptr, &m_SemaphoresForRenderCompleted[i]), "vkCreateSemaphore (2)", __FILE__, __LINE__, __FUNCTION__);

            vk_check(vkCreateFence(m_Driver, &fence_create_info, nullptr, &m_InFlightFences[i]), "vkCreateFence", __FILE__, __LINE__, __FUNCTION__);
        }

        // ConsoleLogWarn("Vulkan Swapchain Sync Objects Initialized Completed!!!!");
        ConsoleLogWarnWithTag("vulkan", "Swapchain Sync Objects Initialized Completed!!!!");

        // ConsoleLogWarn("Vulkan Swapchain Initialized Complete");
        ConsoleLogWarnWithTag("vulkan", "Swapchain Initialized Complete");
        m_IsSwapchainResized = false;
    }
    ...
    uint32_t VulkanSwapchain::ReadAcquiredNextFrame() {
        ...
        auto res = vkAcquireNextImageKHR(m_Driver, m_Swapchain, std::numeric_limits<uint64_t>::max(), m_SemaphoresForAvailableImages[m_CurrentFrameIndex], VK_NULL_HANDLE, &image_index);
    ...
    bool VulkanSwapchain::SwapchainResized(){
        return m_IsSwapchainResized;
    }

    void VulkanSwapchain::ResizeStatus(bool IsResizeSignaled){
        m_IsSwapchainResized = IsResizeSignaled;
    }
