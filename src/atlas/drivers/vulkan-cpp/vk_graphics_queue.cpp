...
namespace atlas::vk {
    ...
    vk_graphics_queue::vk_graphics_queue(
      const vk_queue_options& p_queue_options) {
        //! TODO: Check if we are getting the compute/transfer/graphics queue
        //! through physical driver and then pass the result of the queue family
        //! index into the get_queue that returns VkQueue
    ...
    void vk_graphics_queue::immediate_submit_sync(
    ...
    void vk_graphics_queue::immediate_submit_async(
      const VkCommandBuffer& p_command_buffer) {
        VkSubmitInfo submit_info = {
            .sType = VK_STRUCTURE_TYPE_SUBMIT_INFO,
            .pNext = nullptr,
