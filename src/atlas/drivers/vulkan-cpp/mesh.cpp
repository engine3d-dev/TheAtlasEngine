...
namespace atlas::vk {
    ...
    void mesh::reload_mesh(const std::filesystem::path& p_filename) {
        ...
        // console_log_fatal("LOAD OBJ EXTENSION = {}", p_filename.extension().string());
    ...
    void mesh::load_gltf([[maybe_unused]] const std::filesystem::path& p_filename) {
        ...
        bool is_valid = loader.LoadASCIIFromFile(&model, &error_msg, &warn_msg, p_filename.string());

        if(!is_valid) {
            console_log_fatal("Could not load gltf model from specified path {}", p_filename.string());
            if(!error_msg.empty()) {
        ...
        const tinygltf::Scene scene_loading = model.scenes[model.defaultScene > -1 ? model.defaultScene : 0];
            ...
            if(node.mesh != -1) {
                ...
                for (const auto& primitive : mesh.primitives) {
                    ...
                    if (primitive.mode != TINYGLTF_MODE_TRIANGLES) {
                        // std::cout << "Warning: Primitive mode " << primitive.mode << " not supported. Skipping." << std::endl;
                        continue;
                    }
                    ...
                    size_t pos_stride = pos_accessor->ByteStride(pos_buffer_view); // Will be 12 for vec3f
                    ...
                    if (norm_accessor) {
                        ...
                        const tinygltf::Buffer& norm_buffer = model.buffers[norm_buffer_view.buffer];
                        normals = reinterpret_cast<const float*>(
                            &norm_buffer.data[norm_buffer_view.byteOffset + norm_accessor->byteOffset]);
                        norm_stride = norm_accessor->ByteStride(norm_buffer_view);
                    }
                    ...
                    const float* colors = nullptr; // Assuming float vec3 or vec4 for colors
                    size_t color_stride = 0;
                    if (color_accessor) {
                        const tinygltf::BufferView& color_buffer_view = model.bufferViews[color_accessor->bufferView];
                        const tinygltf::Buffer& color_buffer = model.buffers[color_buffer_view.buffer];
                        // Check component type for color (can be float, ushort, ubyte)
                        if (color_accessor->componentType == TINYGLTF_COMPONENT_TYPE_FLOAT) {
                            colors = reinterpret_cast<const float*>(
                                &color_buffer.data[color_buffer_view.byteOffset + color_accessor->byteOffset]);
                        }
                    ...
                    // size_t current_base_vertex_index = vertices.size(); // To remap glTF indices to our unique vertex indices
                    ...
                    for (size_t k = 0; k < num_indices_to_read; ++k) {
                        ...
                        switch (index_accessor->componentType) {
                            case TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE: {
                                gltf_vertex_idx = static_cast<uint32_t>(index_data[k]);
                                break;
                            }
                            case TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT: {
                                gltf_vertex_idx = static_cast<uint32_t>(reinterpret_cast<const uint16_t*>(index_data)[k]);
                                break;
                            }
                            case TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT: {
                                gltf_vertex_idx = reinterpret_cast<const uint32_t*>(index_data)[k];
                                break;
                            }
                        }
                        ...
                        current_vertex.position = glm::vec3(
                            *(positions + (gltf_vertex_idx * pos_stride / sizeof(float)) + 0),
                            *(positions + (gltf_vertex_idx * pos_stride / sizeof(float)) + 1),
                            *(positions + (gltf_vertex_idx * pos_stride / sizeof(float)) + 2)
                        );

                        // Normal
                        if (norm_accessor && norm_accessor->componentType == TINYGLTF_COMPONENT_TYPE_FLOAT && norm_accessor->type == TINYGLTF_TYPE_VEC3) {
                            current_vertex.normals = glm::vec3(
                                *(normals + (gltf_vertex_idx * norm_stride / sizeof(float)) + 0),
                                *(normals + (gltf_vertex_idx * norm_stride / sizeof(float)) + 1),
                                *(normals + (gltf_vertex_idx * norm_stride / sizeof(float)) + 2)
                            );
                        }
                        else {
                            current_vertex.normals = glm::vec3(0.0f, 0.0f, 0.0f); // Default normal
                        }

                        // UV
                        if (uv_accessor && uv_accessor->componentType == TINYGLTF_COMPONENT_TYPE_FLOAT && uv_accessor->type == TINYGLTF_TYPE_VEC2) {
                            current_vertex.uv = glm::vec2(
                                *(uvs + (gltf_vertex_idx * uv_stride / sizeof(float)) + 0),
                                *(uvs + (gltf_vertex_idx * uv_stride / sizeof(float)) + 1)
                            );
                        }
                        else {
                            current_vertex.uv = glm::vec2(0.0f, 0.0f); // Default UV
                        }

                        // Color
                        if (color_accessor && color_accessor->componentType == TINYGLTF_COMPONENT_TYPE_FLOAT) {
                            if (color_accessor->type == TINYGLTF_TYPE_VEC3) {
                                current_vertex.color = glm::vec3(
                                    *(colors + (gltf_vertex_idx * color_stride / sizeof(float)) + 0),
                                    *(colors + (gltf_vertex_idx * color_stride / sizeof(float)) + 1),
                                    *(colors + (gltf_vertex_idx * color_stride / sizeof(float)) + 2)
                                );
                            }
                            if (color_accessor->type == TINYGLTF_TYPE_VEC4) {
                                // glTF colors are typically linear RGB. If you need sRGB, convert it.
                                current_vertex.color = glm::vec3(
                                    *(colors + (gltf_vertex_idx * color_stride / sizeof(float)) + 0),
                                    *(colors + (gltf_vertex_idx * color_stride / sizeof(float)) + 1),
                                    *(colors + (gltf_vertex_idx * color_stride / sizeof(float)) + 2)
                                );
                                // Alpha component is at index 3 if needed: *(colors + ... + 3)
                            }
                            else {
                                current_vertex.color = glm::vec3(1.0f, 1.0f, 1.0f); // Default white
                            }
                        }
                        else {
                            current_vertex.color = glm::vec3(1.0f, 1.0f, 1.0f); // Default white
                        ...
                        // --- Deduplication ---
                        // If this vertex_input combination already exists, use its index.
                        // Otherwise, add it to our unique_vertices and out_vertices.
