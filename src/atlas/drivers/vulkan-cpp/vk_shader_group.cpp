...
namespace atlas::vk {
    ...
    static VkShaderModule create_shader_module(
      const std::span<uint32_t>& p_binary_blob) {
        ...
        vk_check(vkCreateShaderModule(
                   driver, &shader_module_ci, nullptr, &shader_module),
    ...
    static VkShaderModule create_shader_module(
      const std::span<char>& p_binary_blob) {
        ...
        VkShaderModuleCreateInfo shader_module_ci = {
            ...
            .pNext = nullptr,
            .codeSize = static_cast<uint32_t>(p_binary_blob.size_bytes()),
            .pCode = reinterpret_cast<const uint32_t*>(p_binary_blob.data())
        };
      ...
      const shader_info& p_shader_source) {
        ...
        if (std::filesystem::is_regular_file(filename)) {
            ...
            shaderc::CompilationResult result =
              compiler.CompileGlslToSpv(shader_raw_source.data(),
            ...
            if (result.GetCompilationStatus() !=
                ...
                console_log_error(
                  "{} file had a shader compilation error with status {}",
                  filename,
            ...
            shaderc::CompilationResult result =
              compiler.CompileGlslToSpv(source.data(),
                                        source.size(),
                                        shader_type,
                                        "builtin",
                                        "main",
            ...
            //! @note TODO: Should have a better approach at checking if shaderc
            //! failed to compile shaders
            if (result.GetCompilationStatus() !=
                shaderc_compilation_status_success) {
                console_log_error("Shader compilation with raw shader string "
                                  "with exit status {}",
                                  (int)result.GetCompilationStatus());
                console_log_error("{}", result.GetErrorMessage());
                return {};
        ...
        for (const shader_info& info : m_shader_sources) {
            ...
            if (!source_blobs.empty()) {
                vk_shader_module module_info = {
                    .module_handler = create_shader_module(source_blobs),
                    .stage = info.stage
                };
    ...
    void vk_shader_group::destroy() {
        for (size_t i = 0; i < m_shader_modules.size(); i++) {
            vkDestroyShaderModule(
              m_driver, m_shader_modules[i].module_handler, nullptr);
        }
    }
    ...
    void vk_shader_group::set_vertex_attributes(
      const std::initializer_list<VkVertexInputAttributeDescription>&
        p_attributes) {
        m_vertex_attributes =
          std::vector<VkVertexInputAttributeDescription>(p_attributes);
    }
    ...
    void vk_shader_group::set_vertex_bind_attributes(
      const std::initializer_list<VkVertexInputBindingDescription>&
        ...
        m_vertex_binding_attributes =
          std::vector<VkVertexInputBindingDescription>(p_bind_attributes);
    }
      ...
      const std::span<vertex_attribute>& p_vertex_attributes) {
        ...
        for (size_t i = 0; i < m_vertex_binding_attributes.size(); i++) {
            ...
            for (size_t j = 0; j < attribute.entries.size(); j++) {
                ...
                m_vertex_attributes[j] = { .location = attr_entry.location,
                                           .binding = attribute.binding,
                                           .format =
                                             to_vk_format(attr_entry.format),
