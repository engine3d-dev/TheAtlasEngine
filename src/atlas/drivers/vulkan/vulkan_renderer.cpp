...
namespace atlas::vk {
    ...
    void createBuffer(VkDeviceSize p_size,
        ...
        vk_check(
          vkCreateBuffer(
            vk_context::get_current_driver(), &bufferInfo, nullptr, &p_buffer),
          "vkCreateBuffer",
          __FILE__,
          __LINE__,
          __FUNCTION__);
        ...
        vkGetBufferMemoryRequirements(
          vk_context::get_current_driver(), p_buffer, &memRequirements);
        ...
        allocInfo.memoryTypeIndex =
          vk_context::get_current_selected_physical_driver().search_memory_type(
            memRequirements.memoryTypeBits, p_properties);
        ...
        vkBindBufferMemory(
          vk_context::get_current_driver(), p_buffer, p_buffer_memory, 0);
    }
    ...
    void vk_renderer::initialize_pipeline() {
        ...
        */
        ...
        std::vector<VkDescriptorSetLayout> descriptor_set_layouts = {
            g_global_set_layout->get_descriptor_set_layout()
        };
        ...
        vk::vk_check(vkCreatePipelineLayout(g_driver,
        ...
        vk::vk_check(vkCreateCommandPool(
                       g_driver, &pool_create_info, nullptr, &g_command_pool),
                     "vkCreateCommandPool",
                     __FILE__,
                     __LINE__,
                     __FUNCTION__);
        ...
        vk::vk_check(vkAllocateCommandBuffers(g_driver,
    ...
    void vk_renderer::draw_scene(
      [[maybe_unused]] ref<scene_object> p_scene_ctx) {}

    void vk_renderer::draw_scene_objects(
      [[maybe_unused]] const ref<scene_scope>& p_scene_ctx) {}

    void vk_renderer::draw_scene_object(
      [[maybe_unused]] ref<scene_object>& p_current_scene_object_ctx) {}

    void vk_renderer::submit_scene_object(
      ref<scene_object>& p_current_scene_object_ctx) {
        ...
        // g_CurrentSelectedCamera->get<atlas::PerspectiveCamera>();
        ...
        camera_ubo push_const_data = {
            ...
            .LightPosition =
              (camera_component->Position -
               p_current_scene_object_ctx->get<Transform>()->Position) *
              delta_time,
        };
        // auto frame_idx = get_current_command_buffer_index();

        // memcpy(g_uniform_buffers[frame_idx].uniform_bufferMappedData,
        // &push_const_data, sizeof(push_const_data));
    ...
    /**
    TODO: Here are a few issues that I think is whats causing the second mesh to
    not be rendered

    1.) Because we are binding graphics pipeline. This means that our second
    mesh is probably getting overridden with the first mesh
        - Still need to look into why thats the case (???)


    */
    void vk_renderer::draw_object_with_camera(ref<scene_object>& p_object,
        ...
        // g_global_ubo_list[frame_idx].WriteToIndex(&push_const_data,
        // frame_idx); g_global_ubo_list[frame_idx].FlushIndex(frame_idx);
        ...
        // auto& uniform_buffer =
        // g_uniform_buffers.at(g_current_frame_index.load());
        /*
        ...
        // g_GlobalUbo.WriteToIndex(&push_const_data,
        // g_current_frame_index.load());
        ...
        // g_global_ubo_list[g_current_frame_index.load()].WriteToIndex(&push_const_data,
        // g_current_frame_index.load());
        ...
        if (component == nullptr) {
            console_log_fatal("MeshComponent was invalid that was attempted to "
                              "get in vulkan_renderer!!!");
            return;
        }
