...
namespace atlas::vk{
    void vk_check(VkResult result, const char* p_tag, const char* p_filepath, uint32_t p_line, const char* p_function_name){
        if(result != VK_SUCCESS){
            console_log_info_tagged("vulkan", "VkResult failed taking in {0} file: {1} --- Line: {2} --- In Function: {3}",p_tag, p_filepath, p_line, p_function_name);
            console_log_info_tagged("vulkan", "VkResult returned: {}", (int)result);
        }
    }
    ...
    void vk_check_format(VkFormat p_format, const char* p_filepath, uint32_t p_line, const char* p_function_name){
        if(p_format == VK_FORMAT_UNDEFINED){
            console_log_info_tagged("vulkan", "VkFormat failed in file: {0} --- Line: {1} --- In Function: {2}", p_filepath, p_line, p_function_name);
            console_log_info_tagged("vulkan", "VkFormat was assigned to VK_FORMAT_UNDEFINED");
        }
    }
