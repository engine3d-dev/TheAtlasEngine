...
namespace atlas::vk{
    vk_shader::vk_shader(const std::string& p_VertShader, const std::string& p_FragShader, const shader_pipeline_config& p_Config){
    ...
    void vk_shader::initialize_graphics_pipeline(const std::string& p_VertShader, const std::string& p_FragShader, const shader_pipeline_config& p_Config){
        ...
        vk_check(vkCreateGraphicsPipelines(vk_context::get_current_driver(), nullptr, 1, &graphics_pipeline_create_info, nullptr, &m_graphics_pipeline), "vkCreateGraphicsPipelines", __FILE__, __LINE__, __FUNCTION__);

        console_log_warn("vertex Shader Size == {}", vert.size());
        console_log_warn("Fragment Shader Size == {}", frag.size());
    }

    std::vector<VkVertexInputBindingDescription> vk_shader::get_vertex_input_bind_description(){
    ...
    */
    std::vector<VkVertexInputAttributeDescription> vk_shader::get_vertex_attribute_description(){
    ...
    void vk_shader::initialize_shader_module(const std::vector<char>& p_Bin, VkShaderModule& p_ShaderMod){
        ...
        vk_check(vkCreateShaderModule(vk_context::get_current_driver(), &shader_mod_create_info, nullptr, &p_ShaderMod), "vkCreateShaderModule", __FILE__, __LINE__, __FUNCTION__);

    }

    shader_pipeline_config vk_shader::shader_configuration(uint32_t p_Width, uint32_t p_Height){
