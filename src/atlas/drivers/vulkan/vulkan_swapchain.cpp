...
namespace atlas::vk{
    ...
    vk_swapchain::vk_swapchain(vk_physical_driver p_PhysicalDriver, vk_driver p_Driver, VkSurfaceKHR p_Surface) : m_current_surface(p_Surface), m_physical(p_PhysicalDriver), m_driver(p_Driver){
        ...
        m_presentation_index = p_PhysicalDriver.get_presentation_index(p_Surface);
        vkGetDeviceQueue(m_driver, m_presentation_index, 0, &m_presentation_queue);
        on_create(application_instance::get_window().get_width(), application_instance::get_window().get_height());
    }
    ...
    void vk_swapchain::on_create(uint32_t p_Width, uint32_t p_Height){
        ...
        vk_check(vkGetPhysicalDeviceSurfaceFormatsKHR(m_physical, m_current_surface, &format_count, nullptr), "vkGetPhysicalDeviceSurfaceFormatsKHR", __FILE__, __LINE__, __FUNCTION__);

        formats.resize(format_count);
        vk_check(vkGetPhysicalDeviceSurfaceFormatsKHR(m_physical, m_current_surface, &format_count, formats.data()), "vkGetPhysicalDeviceSurfaceFormatsKHR", __FILE__, __LINE__, __FUNCTION__);
        ...
        vk_check(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(m_physical, m_current_surface, &surface_capabilities), "vkGetPhysicalDeviceSurfaceCapabilitiesKHR", __FILE__, __LINE__, __FUNCTION__);

        //! @note Extracting number of presentation modes.
        uint32_t preset_modes_count;
        std::vector<VkPresentModeKHR> presentation_modes;
        vk_check(vkGetPhysicalDeviceSurfacePresentModesKHR(m_physical, m_current_surface, &preset_modes_count, nullptr), "vkGetPhysicalDeviceSurfacePresentModesKHR", __FILE__, __LINE__, __FUNCTION__);
        presentation_modes.resize(preset_modes_count);
        vk_check(vkGetPhysicalDeviceSurfacePresentModesKHR(m_physical, m_current_surface, &preset_modes_count, presentation_modes.data()), "vkGetPhysicalDeviceSurfacePresentModesKHR", __FILE__, __LINE__, __FUNCTION__);
        ...
        if((surface_capabilities.supportedTransforms & VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR) == 0){
            ...
            if(!expression){
                console_log_error("Surface does not support VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR transformation");
                assert(false);
            }
        }
        ...
        vk_check(vkCreateSwapchainKHR(m_driver, &create_info, nullptr, &m_swapchain), "vkCreateSwapchainKHR", __FILE__, __LINE__, __FUNCTION__);

        // console_log_warn("Vulkan2Showcase: Vulkan swapchain Initialized Complete!!!");
        console_log_warn_tagged("vulkan", "Vulkan swapchain Initialized Complete!!!");

        // console_log_info("Vulkan2Showcase: Begin vk_swapchain Images Initialization");
        console_log_warn_tagged("vulkan", "Begin vk_swapchain Images Initialization");

        uint32_t swapchain_images_count;
        std::vector<VkImage> images;

        //! @note Getting amount of images for this swapchain.
        vk_check(vkGetSwapchainImagesKHR(m_driver, m_swapchain, &swapchain_images_count, nullptr), "vkGetSwapchainImagesKHR", __FILE__, __LINE__, __FUNCTION__);
        ...
        vk_check(vkGetSwapchainImagesKHR(m_driver, m_swapchain, &swapchain_images_count, images.data()), "vkGetSwapchainImagesKHR", __FILE__, __LINE__, __FUNCTION__);
        ...
        for(uint32_t i = 0; i < swapchain_images_count; i++){
            ...
            vk_check(vkCreateImageView(m_driver, &image_view_create_info, nullptr, &m_swapchain_images[i].ImageView), "vkCreateImageView", __FILE__, __LINE__, __FUNCTION__);
        }
        ...
        vk_check(vkCreateCommandPool(m_driver, &cmd_pool_create_info, nullptr, &m_swapchain_command_pool), "vkCreateCommandPool", __FILE__, __LINE__, __FUNCTION__);
        ...
        vk_check(vkAllocateCommandBuffers(m_driver, &cmd_buffer_alloc_info, m_swapchain_command_buffers.data()), "vkAllocateCommandBuffers", __FILE__, __LINE__, __FUNCTION__);

        // console_log_warn("Vulkan2Showcase: Vulkan swapchain CommandBuffers Initiated Completed!!");
        console_log_warn_tagged("vulkan", "Vulkan swapchain CommandBuffers Initiated Completed!!");
        ...
        // console_log_info("Vulkan2Showcase: Creating vk_swapchain renderpass!");
        ...
        vk_check(vkCreateRenderPass(m_driver, &renderpass_create_info, nullptr, &m_swapchain_renderpass), "vkCreateRenderPass", __FILE__, __LINE__, __FUNCTION__);

        // console_log_warn("Vulkan2Showcase: Vulkan swapchain renderpass Initialized Completed!!!");
        console_log_warn_tagged("vulkan", "Vulkan swapchain renderpass Initialized Completed!!!");


        //! @note Initializing Depth Formats.
        // console_log_info("Vulkan2Showcase: Begin Vulkan swapchain Depth Images for swapchains Initializing...");
        console_log_warn_tagged("vulkan", "Begin Vulkan swapchain Depth Images for swapchains Initializing");
        ...
        for(size_t i = 0; i < m_swapchain_depth_images.size(); i++){
            ...
            vk_check(vkCreateImage(m_driver, &image_create_info, nullptr, &m_swapchain_depth_images[i].Image), "vkCreateImage", __FILE__, __LINE__, __FUNCTION__);

            VkMemoryRequirements memory_requirements;
            vkGetImageMemoryRequirements(m_driver, m_swapchain_depth_images[i].Image, &memory_requirements);
            
            VkMemoryAllocateInfo mem_alloc_info = {
                .sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
                .allocationSize = memory_requirements.size,
                .memoryTypeIndex = select_memory_type(memory_properties, memory_requirements.memoryTypeBits, properties_flag)
            };

            vk_check(vkAllocateMemory(m_driver, &mem_alloc_info, nullptr, &m_swapchain_depth_images[i].DeviceMemory), "vkAllocateMemory", __FILE__, __LINE__, __FUNCTION__);

            vk_check(vkBindImageMemory(m_driver, m_swapchain_depth_images[i].Image, m_swapchain_depth_images[i].DeviceMemory, 0), "vkBindImageMemory", __FILE__, __LINE__, __FUNCTION__);
            ...
            vk_check(vkCreateImageView(m_driver, &image_view_create_info, nullptr, &m_swapchain_depth_images[i].ImageView), "vkCreateImageView", __FILE__, __LINE__, __FUNCTION__);
        }

        // console_log_warn("Vulkan2Showcase: Depth Images Initialized Completed!!");
        console_log_warn_tagged("vulkan", "Depth Images Initialized Completed!!");


        //! @note Setting up framebuffers for swapchain.
        // console_log_info("Vulkan2Showcase: Begin Framebuffers Initialization!");
        ...
        for(size_t i = 0; i < get_images_size(); i++){
            std::array<VkImageView, 2> image_attachment = { m_swapchain_images[i].ImageView, m_swapchain_depth_images[i].ImageView};
            ...
            vk_check(vkCreateFramebuffer(m_driver, &fb_create_info, nullptr, &m_swapchain_framebuffers[i]), "vkCreateFramebuffer", __FILE__, __LINE__, __FUNCTION__);
        }

        // console_log_warn("Vulkan2Showcase: Vulkan Framebuffers Initialization Completed!!!!");
        console_log_warn_tagged("vulkan", "Vulkan Framebuffers Initialization Completed!!!!");


        //! @note Setting up Syncrhonization Objects for semaphores/fences.
        // console_log_info("Vulkan2Showcase: Begin Vulkan Sync Objects Initialization");
        console_log_warn_tagged("vulkan", "Begin Vulkan Sync Objects Initialization");
        ...
        for(size_t i = 0; i < MaxFramesInFlight; i++){
            vk_check(vkCreateSemaphore(m_driver, &semaphore_create_info, nullptr, &m_semaphores_images_available[i]), "vkCreateSemaphore (1)", __FILE__, __LINE__, __FUNCTION__);

            vk_check(vkCreateSemaphore(m_driver, &semaphore_create_info, nullptr, &m_semaphores_render_completed[i]), "vkCreateSemaphore (2)", __FILE__, __LINE__, __FUNCTION__);

            vk_check(vkCreateFence(m_driver, &fence_create_info, nullptr, &m_swapchain_in_flight_fences[i]), "vkCreateFence", __FILE__, __LINE__, __FUNCTION__);
        }

        // console_log_warn("Vulkan swapchain Sync Objects Initialized Completed!!!!");
        console_log_warn_tagged("vulkan", "swapchain Sync Objects Initialized Completed!!!!");
    ...
    void vk_swapchain::submit_and_write_command_buffer(VkCommandBuffer* p_CommandBuffers) {
        // if(m_IsSwapchainResized){
        //     return;
        // }

        if(m_swapchain_fences[m_current_image_index] != VK_NULL_HANDLE){
            vkWaitForFences(m_driver, 1, &m_swapchain_fences[m_current_image_index], true, std::numeric_limits<uint64_t>::max());
        }

        m_swapchain_fences[m_current_image_index] = m_swapchain_in_flight_fences[m_current_frame_index];
        ...
        VkSemaphore wait_semaphore[] = {m_semaphores_images_available[m_current_frame_index]};
        ...
        VkSemaphore signal_sems[] = { m_semaphores_render_completed[m_current_frame_index]};

        submit_info.signalSemaphoreCount = 1;
        submit_info.pSignalSemaphores = signal_sems;

        vkResetFences(m_driver, 1, &m_swapchain_in_flight_fences[m_current_frame_index]);

        

        vk_check(vkQueueSubmit(m_driver.get_graphics_queue(), 1, &submit_info, m_swapchain_in_flight_fences[m_current_frame_index]), "vkQueueSubmit", __FILE__, __LINE__, __FUNCTION__);
        ...
        if(res == VK_ERROR_OUT_OF_DATE_KHR){
            ...
            console_log_error("m_IsSwapchainResized = {}", m_is_resized_requested);
            on_create(application_instance::get_window().get_width(), application_instance::get_window().get_height());
            return;
        }
        ...
        m_current_frame_index = (m_current_frame_index + 1) % vk_swapchain::MaxFramesInFlight;
    }
    
    uint32_t vk_swapchain::read_acquire_next_frame() {
        uint32_t image_index;
        vk_check(vkWaitForFences(m_driver, 1, &m_swapchain_in_flight_fences[m_current_frame_index], true, std::numeric_limits<uint32_t>::max()), "vkWaitForFences", __FILE__, __LINE__, __FUNCTION__);

        auto res = vkAcquireNextImageKHR(m_driver, m_swapchain, std::numeric_limits<uint64_t>::max(), m_semaphores_images_available[m_current_frame_index], VK_NULL_HANDLE, &image_index);
        ...
        vk_check(res, "vkAcquireNextImageKHR", __FILE__, __LINE__, __FUNCTION__);
    ...
    uint32_t vk_swapchain::select_memory_type(VkPhysicalDeviceMemoryProperties p_MemoryProperties, uint32_t p_TypeFilter, VkMemoryPropertyFlags property_flag){
        ...
        console_log_error("vk_swapchain::select_memory_type returns zero meaning failed to find suitable memory type");
        return 0;
    }

    VkPresentModeKHR vk_swapchain::select_compatible_present_mode(const VkPresentModeKHR& p_RequestMode, const std::vector<VkPresentModeKHR>& p_Modes){
    ...
    VkExtent2D vk_swapchain::select_valid_extent(const VkSurfaceCapabilitiesKHR& p_SurfaceCapabilities){
    ...
    VkFormat vk_swapchain::select_depth_format(const VkPhysicalDevice& p_PhysicalDevice){
        return select_supported_format(p_PhysicalDevice,
    ...
    VkFormat vk_swapchain::select_supported_format(VkPhysicalDevice p_PhysicalDevice, const std::vector<VkFormat>& p_Formats, VkImageTiling p_Tiling, VkFormatFeatureFlags p_FeatureFlag){
