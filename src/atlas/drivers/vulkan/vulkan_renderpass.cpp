...
namespace atlas::vk{
    ...
    static VkFormat to_vulkan_format(ImageFormat Format){
        switch (Format){
        case ImageFormat::RGBA16: return VK_FORMAT_R8G8B8A8_SINT;
        default: break;
        }
    ...
    //! @note Used to convert from our enum of ImageLayout into vulkan or an api-specific equivalent of the supported image layouts 
    static VkImageLayout to_image_layout(ImageLayout Layout){
        VkImageLayout layout_value = VK_IMAGE_LAYOUT_UNDEFINED;
        
        switch (Layout){
        case ImageLayout::IMAGE_OPTIMAL: return VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL;
        default: break;
        }
    ...
    /*
        Usage:
            - Helper function for bitwise-OR for knowing what our image layout is going to be
            - Also used to 
    */
    ...
    VulkanRenderPass::VulkanRenderPass(const renderpass_properties& p_Properties){
        auto attachments = p_Properties.attachments;

        //! @note If we have attachments specified for this render-pass
        //! @note If there are not render pass attachments, then we assign defaulted attachments for basic-uses
