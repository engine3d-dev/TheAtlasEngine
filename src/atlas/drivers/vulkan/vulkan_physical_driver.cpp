...
namespace atlas::vk {
    vk_physical_driver::vk_physical_driver(const VkInstance& p_instance) {
        console_log_info(
          "Vulkan: Begin Vulkan Physical Driver Initialization!!!");

        uint32_t gpu_count;
        std::vector<VkPhysicalDevice> available_gpus;

        //! @note Querying number of available GPU's (Physical Devices) on our
        //! machine
        vk_check(vkEnumeratePhysicalDevices(p_instance, &gpu_count, nullptr),
        ...
        vk_check(vkEnumeratePhysicalDevices(
                   p_instance, &gpu_count, available_gpus.data()),
                 "vkEnumeratePhysicalDevices",
                 __FILE__,
                 __LINE__,
                 __FUNCTION__);
        ...
        //! @note Checking for compatible GPU's on our machine. (Init the
        ...
        for (const auto& device : available_gpus) {
            ...
            //! @note VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU checks if our device
        ...
        vkGetPhysicalDeviceQueueFamilyProperties(
          m_physical, &queue_family_count, nullptr);
        m_queue_family_properties.resize(queue_family_count);

        //! @note Loading queue family data from the physical device that we
        //! have selected.
        vkGetPhysicalDeviceQueueFamilyProperties(
          m_physical, &queue_family_count, m_queue_family_properties.data());

        //! @note Getting our actual physical device properties that have been
        //! currently selected.
        ...
        console_log_warn(
          "Vulkan: End Vulkan Physical Initialization Completed");
    }
    ...
    vk_physical_driver::select_queue_family_indices() {
        ...
        // Graphics if its -1 meaning it could not find any graphics queue
        // available. console_log_warn("Graphics Index === {}",
        // (int)indices.Graphics);

        return indices;
    }

    uint32_t vk_physical_driver::search_memory_type(
      uint32_t p_type_filter,
      VkMemoryPropertyFlags p_property_flags) {
        VkPhysicalDeviceMemoryProperties physical_device_memory_properties;
        vkGetPhysicalDeviceMemoryProperties(m_physical,
        ...
        for (uint32_t i = 0;
            ...
            if ((p_type_filter & (1 << i)) and
                (physical_device_memory_properties.memoryTypes[i]
                   .propertyFlags &
                 p_property_flags)) {
                return i;
            }
        ...
        for (const auto& queue_family : m_queue_family_properties) {
            if (queue_family.queueFlags & VK_QUEUE_GRAPHICS_BIT) {
                vk_check(vkGetPhysicalDeviceSurfaceSupportKHR(
                           m_physical, i, p_surface, &compatible),
