...
namespace atlas::vk{
    vk_index_buffer::vk_index_buffer(const std::vector<uint32_t>& p_Indices){
        // console_log_info("Vulkan2Showcase: Begin Vulkan vertex_buffer Initialization!!");
        ...
        if(!m_has_indices){
            console_log_warn("Cannot provide empty indices to the vk_index_buffer!");
            return;
        }
        ...
        vk_check(vkCreateBuffer(vk_context::get_current_driver(), &vk_buffer_create_info, nullptr, &m_index_buffer_handler), "vkCreateBuffer", __FILE__, __LINE__, __FUNCTION__);

        VkMemoryRequirements memory_reqs;
        vkGetBufferMemoryRequirements(vk_context::get_current_driver(), m_index_buffer_handler, &memory_reqs);
        ...
        vk_check(vkAllocateMemory(vk_context::get_current_driver(), &alloc_info, nullptr, &m_index_device_buffer_memory), "vkAllocateMemory", __FILE__, __LINE__, __FUNCTION__);

        vkBindBufferMemory(vk_context::get_current_driver(), m_index_buffer_handler, m_index_device_buffer_memory, 0);
        ...
        */
        void* data;
        vkMapMemory(vk_context::get_current_driver(), m_index_device_buffer_memory, 0, buffer_size, 0, &data);
        memcpy(data, p_Indices.data(), static_cast<size_t>(buffer_size));
        vkUnmapMemory(vk_context::get_current_driver(), m_index_device_buffer_memory);
    ...
    void vk_index_buffer::bind_to_index_buffer(const VkCommandBuffer& p_CommandBuffer) {
        // VkBuffer buffers[] = {m_index_buffer_handler};
        // VkDeviceSize offsets[] = {0};

        if(m_has_indices){
            vkCmdBindIndexBuffer(p_CommandBuffer, m_index_buffer_handler, 0, VK_INDEX_TYPE_UINT32);
        }
    }

    void vk_index_buffer::render_index_buffer(const VkCommandBuffer& p_CommandBuffer) {
        /**
        //! @note This will happen within Renderer itself, but not here nor vertex_buffer class.
        if(m_has_indices){
