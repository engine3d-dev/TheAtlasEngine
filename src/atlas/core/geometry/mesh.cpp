...
namespace std{
    ...
    struct hash<atlas::vertex>{
        size_t operator()(const atlas::vertex& vertex) const {
			size_t seed = 0;
            atlas::hash_combine(seed, vertex.Position, vertex.Color, vertex.Normals, vertex.TexCoords);
            return seed;
		}
	};
...
namespace atlas{
    ...
    Mesh::Mesh(const ref<vertex_buffer>& p_vertex_buffer, const ref<index_buffer>& p_index_buffer){
    ...
    bool Mesh::is_loaded() const { return g_loaded_successful; }
