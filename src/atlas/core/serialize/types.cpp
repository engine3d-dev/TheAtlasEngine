...
namespace atlas {
    ...
    YAML::Emitter& operator<<(YAML::Emitter& p_output,  const physics::collider_body* p_collider) {
        ...
        p_output << YAML::BeginMap;
        p_output << YAML::Key << "Enabled" << YAML::Value << p_collider->collision_enabled;
        p_output << YAML::Key << "Shape Type" << YAML::Value << (int)p_collider->shape_type;
        p_output << YAML::Key << "Extent" << YAML::Value << p_collider->half_extents;
        p_output << YAML::Key << "Radius" << YAML::Value << p_collider->radius;
        p_output << YAML::Key << "Capsule Height" << YAML::Value << p_collider->capsule_half_height;
        p_output << YAML::Key << "Body ID" << YAML::Value << p_collider->body_id;
        p_output << YAML::EndMap;
    ...
    YAML::Emitter& operator<<(YAML::Emitter& p_output,  const physics::physics_body* p_body) {
        ...
        p_output << YAML::BeginMap;
        p_output << YAML::Key << "Linear Velocity" << YAML::Value << p_body->linear_velocity;
        p_output << YAML::Key << "Angular Velocity" << YAML::Value << p_body->angular_velocity;
        p_output << YAML::Key << "Cumulative Force" << YAML::Value << p_body->cumulative_force;
        p_output << YAML::Key << "Cumulative Torque" << YAML::Value << p_body->cumulative_torque;
        p_output << YAML::Key << "Mass Factor" << YAML::Value << p_body->mass_factor;
        p_output << YAML::Key << "Center Mass Position" << YAML::Value << p_body->center_mass_position;
        p_output << YAML::Key << "Gravity Enabled" << YAML::Value << p_body->use_gravity;
        p_output << YAML::Key << "Gravity Factor" << YAML::Value << p_body->gravity_factor;
        p_output << YAML::Key << "Body Type" << YAML::Value << p_body->body_type;
        p_output << YAML::Key << "Friction" << YAML::Value << p_body->friction;
        p_output << YAML::Key << "Restitution" << YAML::Value << p_body->restitution;
        p_output << YAML::Key << "Body Movement Type" << YAML::Value << p_body->body_movement_type;
        p_output << YAML::Key << "Body Layer Type" << YAML::Value << p_body->body_layer_type;
