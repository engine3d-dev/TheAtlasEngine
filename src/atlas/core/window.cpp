...
namespace atlas{
    ...
    ref<window> window::create(uint32_t Width, uint32_t Height, const std::string& Tag){
        ...
        default:
            console_log_error("API that was input was not specifiying valid backend!");
            return nullptr;
        }
    ...
    uint32_t window::get_width() const { return g_WindowWidth; }

    uint32_t window::get_height() const { return g_WindowHeight; }

    bool window::is_active() const { return !glfwWindowShouldClose(get_native_window()); }
    ...
    float window::get_aspect_ratio() const { return (float)get_width() / get_height(); }
