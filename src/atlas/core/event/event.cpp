...
namespace atlas::event {
    ...
    void update_events() {
        ...
        // ....
        //! @note Must be called per input updated events. In the case either
        //! game console disconnects or reconnects
        //! @note This will continously check.
        //! @note By default GLFW check's up to a total of 16 joystick ID's that
        //! are available
        //! @note We iterate all 16 joysticks, only using the joystic ID's that
        //! are connected
        //! @note Then checking for any events from the connected joystick has
        //! occurred
        ...
        for (int joystick_id = 0; joystick_id < 16; joystick_id++) {
            if (glfwJoystickPresent(joystick_id) == GLFW_TRUE) {
                ...
                //! @note We always check how many buttons the joysticks that
                //! are connected contain.
