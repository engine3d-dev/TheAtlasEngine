...
namespace atlas{
    ...
    void system_registry::register_to(world_scope* p_world){
        ...
        console_log_info( "register_to::g_CurrentWorld->get_tag() = {}", current_world.get_tag());
    }
    ...
    /*
        ...
        scene_scope
            ...
            - How the scene_scope gets to know the world its associated is when you register_to the scene doing, system_registry::register_to(this)
            - This can be throw some API like, system_registry::get_world(this), where we fetch the scene scope's hash UUID
