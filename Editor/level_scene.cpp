...
/**
    ...
    - Notes for figuring out what commonalities that can be visualized for basic properties to get basic physics working using Jolt
...
void InitializeJoltAPI(){
    ...
    temp_allocator = atlas::create_ref<JPH::TempAllocatorImpl>(10 * 1024 * 1024);
    job_system = atlas::create_ref<JPH::JobSystemThreadPool>(JPH::cMaxPhysicsJobs, JPH::cMaxPhysicsBarriers, std::thread::hardware_concurrency() - 1);
    ...
    atlas::physics::ObjectVsBPLayerFilterInterface object_vs_broadphase_layer_filter;

    // Create class that filters object vs object layers
    // Note: As this is an interface, PhysicsSystem will take a reference to this so this instance needs to stay alive!
    // Also have a look at ObjectLayerPairFilterTable or ObjectLayerPairFilterMask for a simpler interface.
    atlas::physics::ObjectLayerPairFilterInterface object_vs_object_layer_filter;
...
namespace ui{
    [[maybe_unused]] static bool BeginPopupContextWindow(const char* str_id, ImGuiMouseButton mb, bool over_items) { return ImGui::BeginPopupContextWindow(str_id, mb | (over_items ? 0 : ImGuiPopupFlags_NoOpenOverItems)); }

    [[maybe_unused]] static void draw_entity_transform(const atlas::Transform& p_transform){
        atlas::Transform read_transform = p_transform;
        atlas::ui::draw_vec3("pos 1", read_transform.Position);
        atlas::ui::draw_vec3("scale 1", read_transform.Scale);
        atlas::ui::draw_vec3("rotate 1", read_transform.Rotation);
...
level_scene::level_scene(const std::string& p_tag) : atlas::scene_scope(p_tag){
    ...
    console_log_trace("From level_scene::level_scene() ==> World Tag = {}", world_tag);
    ...
    */
    ...
    console_log_warn("World Tag After Initialization ======>>>>> {}", atlas::system_registry::get_world().get_tag());
}

void level_scene::on_ui_update(){
    
    if(ImGui::Begin("Viewport")){
        glm::vec2 viewportPanelSize = glm::vec2{atlas::application_instance::get_window().get_width(), atlas::application_instance::get_window().get_height()};
    ...
    if(ImGui::Begin("Scene Heirarchy")){
		...
		if(ui::BeginPopupContextWindow(0, 1, false)){
			if(ImGui::MenuItem("Create Empty Entity")){
                // m_scene_objects.insert({"Empty Entity", this->create_new_object("Empty Object")});
            }
			ImGui::EndPopup();
		}
        ImGui::End();
    ...
    if(ImGui::Begin("Properties Panel")){
        ...
        atlas::ui::draw_panel_component<atlas::MeshComponent>("Sphere", [&](){
            ...
            // sphere_data.mesh_file = atlas::ui::button_open_file_dialog("Load Mesh 1");
            ...
            if(sphere_data.mesh_file != ""){
                ...
                // m_sphere->set<MeshComponent>({"3d_models\\tutorial\\Ball OBJ.obj"});
        ...
        atlas::ui::draw_panel_component<atlas::MeshComponent>("Some Mesh", [&](){
            ...
            if(some_mesh_data.mesh_file != ""){
                ...
                console_log_trace("in branch 2 mesh_file = {}", relative_path.filename().string());
                m_platform->set<atlas::MeshComponent>({some_mesh_data.mesh_file});
                some_mesh_data.mesh_file = "";
            }
        });

        atlas::ui::draw_panel_component<atlas::PerspectiveCamera>("Camera", [&](){
            atlas::ui::draw_vec3("Position", camera_data.Position);
            atlas::ui::draw_vec3("Front", camera_data.Front);
        });
...
void level_scene::on_update() {
    ...
    //! TODO: global_update just uses delta timer for frame-rate stuff, and we should move that in timer namespace
    ...
    if (atlas::event::is_key_pressed(KEY_ESCAPE)){
        atlas::application_instance::get_window().close();
    }
    ...
    m_sphere->set<atlas::Transform>({
        ...
        // .Color = sphere
        .Color = {sphere_data.Color.x, sphere_data.Color.y, sphere_data.Color.z, 1.0f}
    });
    ...
    m_platform->set<atlas::Transform>({
        ...
        // .Color = platform
        .Color = {some_mesh_data.Color.x, some_mesh_data.Color.y, some_mesh_data.Color.z, 1.0f}
    });
...
void level_scene::on_physics_update(){
    ...
    //! @note This would be replaced with the play button. Once we get textures into the mix (after getting the viewport settled)
...
void level_scene::on_runtime_start(){
    /*
        ...
        // Every time the transforms get modified then these will also be modified as well
        m_physics_scene.add_entity(m_sphere);
...
void level_scene::on_runtime_stop(){
