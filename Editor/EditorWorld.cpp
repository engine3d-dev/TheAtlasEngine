...
EditorWorld::EditorWorld() : m_Scene1("Scene 1"){
...
void EditorWorld::OnUpdate(){
    ...
    if(glm::dot(rotate, rotate) > std::numeric_limits<float>::epsilon()){
        float dt = engine3d::SyncUpdateManager::DeltaTime();
    ...
    if(glm::dot(move_dir, move_dir) > std::numeric_limits<float>::epsilon()){
        transform.Position += s_MoveSpeed * (engine3d::SyncUpdateManager::DeltaTime()) * glm::normalize(move_dir) * pos_sensitivity;
    }
